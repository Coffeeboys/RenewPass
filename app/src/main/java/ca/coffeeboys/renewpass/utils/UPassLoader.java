...
import ca.coffeeboys.renewpass.exceptions.NothingToRenewException;
import ca.coffeeboys.renewpass.exceptions.SchoolAuthenticationFailedException;
import ca.coffeeboys.renewpass.exceptions.SchoolNotFoundException;
import ca.coffeeboys.renewpass.model.Callback;
import ca.coffeeboys.renewpass.model.Status;
import ca.coffeeboys.renewpass.schools.School;
import ca.coffeeboys.renewpass.schools.SimonFraserUniversity;
import ca.coffeeboys.renewpass.schools.UniversityOfBritishColumbia;
...
public class UPassLoader {
    ...
    private class RenewTask extends AsyncTask<Boolean, Void, Status> {
        ...
        @Override
        protected ca.coffeeboys.renewpass.model.Status doInBackground(Boolean... params) {
            Boolean doRenew = params[0];
            ca.coffeeboys.renewpass.model.Status returnStatus;
            try {
                ...
                if (doRenew) {
                    ...
                    if (requestSuccess) {
                        returnStatus = new ca.coffeeboys.renewpass.model.Status(ca.coffeeboys.renewpass.model.Status.RENEW_SUCCESSFUL, true);
                    }
                    else {
                        returnStatus = new ca.coffeeboys.renewpass.model.Status(ca.coffeeboys.renewpass.model.Status.RENEW_FAILED, false);
                    }
                }
                else {
                    returnStatus = new ca.coffeeboys.renewpass.model.Status(ca.coffeeboys.renewpass.model.Status.UPASS_AVAILABLE, true);
                }
            }
            catch(SchoolNotFoundException e) {
                return new ca.coffeeboys.renewpass.model.Status(ca.coffeeboys.renewpass.model.Status.SCHOOL_NOT_FOUND, false);
            }
            catch(SchoolAuthenticationFailedException e) {
                return new ca.coffeeboys.renewpass.model.Status(ca.coffeeboys.renewpass.model.Status.AUTHENTICATION_ERROR, false);
            }
            catch(NothingToRenewException e) {
                return new ca.coffeeboys.renewpass.model.Status(ca.coffeeboys.renewpass.model.Status.NOTHING_TO_RENEW, true);
            }
            catch(MechanizeException e) {
                return new ca.coffeeboys.renewpass.model.Status(ca.coffeeboys.renewpass.model.Status.NETWORK_ERROR, false);
            }
            catch(Exception e) {
                return new ca.coffeeboys.renewpass.model.Status(ca.coffeeboys.renewpass.model.Status.UNKNOWN_ERROR, false);
            }
        ...
        @Override
        protected void onPostExecute(ca.coffeeboys.renewpass.model.Status result) {
